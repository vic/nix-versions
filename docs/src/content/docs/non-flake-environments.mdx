...
nixpkgs with `fetchurl`. Doing so can consume resources (time and network) on our server.
And given that many people are moving to using flakes, we are marking non-flakes features
as _experimental_.
:::
